...
grammar for values that are nats or errors

nat_or_error ::= Error | NotError nat
...
Definition pred_option_nat (n : nat) : option_nat := 
  match n with
  | O => None_nat
  | S m => Some_nat m
  end.

Definition pred_bogus (n : nat) : nat := 
    match n with
    | O => O
    | S m => m
    end. 
(*
forall n, n <> 0 -> pred_bogus n < n
*)
Inductive option (A : Set) : Set :=
| None : option A
| Some : A -> option A.

Definition pred_underscore (n : nat) : option nat := 
  match n with
  | O => None _
  | S m => Some _ m
  end.
...
Check None.
(*
Arguments None _ : clear implicits.
*)
...
Fixpoint app {A : Set} (l1 : list A) (l2 : list A)
  : list A :=
  match l1 with 
  | nil => l2
